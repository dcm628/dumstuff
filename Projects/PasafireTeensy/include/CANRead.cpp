...
bool CANread(FlexCAN& CANbus, Command& CurrentCommand)
{
    ...
    if(CANbus.available())
    {
        if(CANbus.read(msgIn))                                                                // read occurs inside if statement
        {
            NewMessage = true;                                                              // set new message flag to true if message recieved and read

            // add CAN messages to internal buffer
            for(uint8_t index{0}; index < msgIn.len; ++index)                                 // restrict to length of message
            {
                if(msgIn.buf[index] < command_SIZE)                                           // this checks if the message at that location in the buffer could be a valid command
                {
                    // convert message to a Command type and save it to the buffer
                    CommandBuffer.at(CommandBufferIndex) = static_cast<Command>(msgIn.buf[index]);
